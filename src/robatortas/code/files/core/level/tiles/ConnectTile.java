...
public class ConnectTile {
	...
	public void init() {
		...
		if(below() && connects()) screen.renderSprite(x << 4, y << 4, below, 1, 0);
		
		if(down && right || down && left || up && right || up && left) {
			if(up && right) screen.renderSprite(xt, yt, urSprite, 1, 0); 
			else if(up && left) screen.renderSprite(xt, yt, ulSprite, 1, 0);
		
			if(down && right) screen.renderSprite(xt, yt, drSprite, 1, 0);
			else if(down && left) screen.renderSprite(xt, yt, dlSprite, 1, 0);
		} else if(up || down || left || right){
			if(up) screen.renderSprite(xt, yt, upSprite, 1, 0);
			if(down) screen.renderSprite(xt, yt, downSprite, 1, 0);
			if(left) screen.renderSprite(xt, yt, leftSprite, 1, 0);
			if(right) screen.renderSprite(xt, yt, rightSprite, 1, 0);
		}
	}
