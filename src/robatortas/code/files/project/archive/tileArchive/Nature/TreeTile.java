...
public class TreeTile extends TileManager {
	...
	public void hurting(LevelManager level, int x, int y, int damage) {
		level.tileStates[x + y * level.width].health += damage;
		...
		if(level.tileStates[x + y * level.width].health >= 10) {
			level.tileData[x + y * level.width] = 0;
	...
	public void render(int x, int y, LevelManager level, RenderManager screen) {
		Random posRandom = new Random(x * 5L + y * 7L);
		int offsetX = posRandom.nextInt(5) - 3;
		int offsetY = posRandom.nextInt(5) - 2;
		screen.renderTile(((x - 1) << 4) + 7 + offsetX, ((y-1) << 4) - 5 + offsetY, 1.2f, this);
	}
