...
public class InventoryMenu {
	...
	public void render(RenderManager screen, GameManager game) {
		...
		if (game.mouse != null) grid = game.mouse.toggle(MouseManager.LEFT, grid);
		if(grid) clickBoxes(screen, game);
	}
	
	public void clickBoxes(RenderManager screen, GameManager game) {screen.renderBox(10, 10, 10, 10, 0xff0000ff, 255, true);
		// Big pocket
		if(MouseManager.mX >= 329 && MouseManager.mX <= 406 && MouseManager.mY >= 222 && MouseManager.mY <= 344) {
			new BigPocket().render(screen, game);
			screen.renderBox(10, 10, 10, 10, 0xff0000ff, 255, true);
		}
